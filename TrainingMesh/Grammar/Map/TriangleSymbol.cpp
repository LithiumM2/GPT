...
void TriangleSymbol::Generate(Mesh &mesh, int level) const {
	if ((level == 0)) {
		...
		if (checkNormal(Triangle(p1, p2, p3))) {
			...
			Mesh m = Mesh::Triangle(t.getPoints()[0], t.getPoints(
				)[1], t.getPoints()[2]);
			m.merge(Mesh::RouteL(p2, p1, t.getPoints()[0],
				t.getPoints()[1], 3.f, 1.f));
			m.merge(Mesh::RouteL(p1, p3, t.getPoints()[2],
				t.getPoints()[1], 3.f, 1.f));
			m.merge(Mesh::RouteL(p3, p2, t.getPoints()[1],
			...
			m.merge(Mesh::RouteL(p1, p2, t.getPoints()[1],
				t.getPoints()[0], 3.f, 1.f));
			m.merge(Mesh::RouteL(p2, p3, t.getPoints()[2],
				t.getPoints()[1], 3.f, 1.f));
		...
		//20000.f ) { Rendu mauvais
			...
			//mesh, level - 1 );
			//}
		else if (random < 45 && Triangle(p1, p2, p3).area() >
			5000.f) {
		...
		//	static_cast <float> (RAND_MAX / (.6f - .4f)) );
			...
			if (a >= c && b >= c) {
				...
				if (checkNormal(Triangle(p2, p4, p5))) {
					TriangleSymbol(p4, p2, p5, mid, loin).Generate(
						mesh, level - 1);
					QuadrangleSymbol(p1, p3, p4, p5, mid, loin
						).Generate(mesh, level - 1);
				}
				else {
					TriangleSymbol(p2, p4, p5, mid, loin).Generate(
						mesh, level - 1);
					QuadrangleSymbol(p1, p5, p4, p3, mid, loin
						).Generate(mesh, level - 1);
				}
			}
			else if (b >= a && c >= a) {
				...
				if (checkNormal(Triangle(p1, p5, p4))) {
					TriangleSymbol(p5, p1, p4, mid, loin).Generate(
						mesh, level - 1);
					QuadrangleSymbol(p2, p3, p5, p4, mid, loin
						).Generate(mesh, level - 1);
				}
				else {
					TriangleSymbol(p1, p5, p4, mid, loin).Generate(
						mesh, level - 1);
					QuadrangleSymbol(p2, p4, p5, p3, mid, loin
						).Generate(mesh, level - 1);
				}
			}
			else if (c >= b && a >= b) {
				...
				if (checkNormal(Triangle(p3, p4, p5))) {
					TriangleSymbol(p4, p3, p5, mid, loin).Generate(
						mesh, level - 1);
					QuadrangleSymbol(p2, p5, p4, p1, mid, loin
						).Generate(mesh, level - 1);
				}
				else {
					TriangleSymbol(p3, p4, p5, mid, loin).Generate(
						mesh, level - 1);
					QuadrangleSymbol(p2, p1, p4, p5, mid, loin
						).Generate(mesh, level - 1);
				}
			}
		}
		...
		//	triangles
			...
			if (rng == 0) {
				...
				c.radius -= (c.radius * static_cast < float > (rand(
			...
			if (rng == 1) {
				...
				c0.radius -= (c0.radius * static_cast < float > (rand
					()) / static_cast < float > (RAND_MAX));
				Quadrangle q = randomQuadInCircle(c0);
				RDC(q.p2, q.p1, q.p4, q.p3, 3.0f, dif, 0).G(mesh);

				Circle c1 = incircle(Triangle(t2[1], t2[0], t2[2]));
				c1.radius -= (c1.radius * static_cast < float > (rand
					()) / static_cast < float > (RAND_MAX));
			...
			if (rng == 2) {
				...
				c2.radius -= (c2.radius * static_cast < float > (rand
					()) / static_cast < float > (RAND_MAX));
			...
			TriangleSymbol(p1, p2, p3, mid, loin).Generate(mesh,
				level - 1);
		}
}
	}
	...
	Quadrangle TriangleSymbol::randomQuadInCircle(Circle c) {
		...
		Quadrangle q(
			Vec3<float>(c.center.x - std::cos(angle1) * c.radius,
			c.center.y + std::sin(angle1) * c.radius, c.center.z),
			Vec3<float>(c.center.x + std::cos(angle2) * c.radius,
			c.center.y - std::sin(angle2) * c.radius, c.center.z),
			Vec3<float>(c.center.x - std::cos(angle3) * c.radius,
			c.center.y + std::sin(angle3) * c.radius, c.center.z),
			Vec3<float>(c.center.x + std::cos(angle4) * c.radius,
			c.center.y - std::sin(angle4) * c.radius, c.center.z));
